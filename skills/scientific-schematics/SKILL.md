...
description: "[阶段1·示意图生成] Generate publication-grade CONCEPT/SCHEMATIC figures (flowcharts, pipelines, mechanism diagrams) via iterative AI-image-generation + independent vision scoring. Use when a figure is a drawing/schematic rather than a data plot. NOT for data panels (use sci-figure-plot) or composition (sci-figure-compose). Methodology adapted from davila7/claude-code-templates scientific-schematics (score-loop), local chain: GPT DALL-E via cli-anything-gpt + vision review via analyze_image."
---
...
## 工作流（评分迭代环，adapted from davila7 scientific-schematics）
...
## 关键判据：整幅生成 vs 素材组合（2026-08-17 Fig1 五版学费）
...
- 生图链路：GPT-CDP（cli-anything-gpt，可用但实例偶抽风）；
  本地闭环待配置图像生成 API key（以 zai-sdk 为例，images.generate 就绪）

## 实测配方（2026-08-17 Fig1 素材，4/4 过检）
