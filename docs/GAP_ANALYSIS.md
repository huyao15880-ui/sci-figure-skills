...
## G9 3D 图能力为零（2026-08-17 新增，最大增量缺口）

**现状**：全仓库无一个 3D 配方；而三领域 3D 需求高频——纳米材料
（晶体结构/表面形貌/等值面）、生物（蛋白/分子结构）、艺术设计
（3D 封面渲染，千元级服务区间）。
**对标**：[PyVista](https://pyvista.org/)（VTK 现代封装，脚本化可复现
出版图，活跃维护）为 3D 数据图主力；OVITO 独占原子/晶体模拟渲染；
Mayavi 维护弱于 PyVista；Blender headless 覆盖艺术渲染。
**路径**：见 `CHART_CATALOG.md` 第三节与学习路线——P1 先补
PyVista 曲面/等值面 + OVITO 晶体管线。
...
| P0 | G5 三领域各 3-5 配方（谱图族/雨云森林见 CHART_CATALOG P0） | 服务菜单的最小可售集 |
| P1 | G1 样式包编译 | 分发壳（PyPI 引流） |
| P1 | G8 确定性重叠检测 | 降本（服务毛利） |
| P1 | G9 3D 最小集（PyVista + OVITO） | 千元级定价区间的入场券 |
