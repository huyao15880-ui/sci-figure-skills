...
## 七点九、拟合质量研究：用户抓到"没拟合好"（recipe 10）

单壳层 Pt-N 拟合在 R 1.0-2.2 内残差 1.2% 但高壳层 67% 未建模——
全范围图上拟合线在 2.3+ 平掉，视觉"没拟合好"（用户在 Origin 里看出）。
系统研究（一次一变量）：

- 公平基线：同窗 1.0-3.2 下单路径 Rf=0.0539
- 加第二壳层 C@3.0（双路径）：Rf→0.034（高壳层残差 67%→12%）
- **rbkg 敏感性（关键发现）**：0.8→0.022 / 1.0→0.037 / **1.25→0.020**
  / 1.5→0.141（崩溃）——rbkg=1.25 即 Athena 默认，前人报告大概率用它
- 终局配置：rbkg=1.25 + σ²_C 自由 + 双壳层 + 全窗 → **Rf=0.0169 < 0.02
  达标**（前人 0.013）；Pt-N 2.40@1.989 Å + 第二壳层 C 1.28@2.926 Å，
  总配位 ~3.7 与报告 4.3±0.5 在 1σ 内
- 壳层残差 0.5% / 4.7%，全范围视觉贴合；模板 opju Graph2 已刷新

**教学点**：Rf 对比必须同拟合窗；rbkg 是隐藏的最大敏感参数，投稿答辩
要报 rbkg 值；"拟合好"的视觉判据 = 全范围 data/fit 贴合，不只主峰。

## 七点十、什么才是好的拟合（用户二次质疑后的完整答案）

用户两次抓拟合问题。第一次对（高壳层未建模）；第二次的答案分层：

**贴合层（已达标）**——好拟合的金标准三重贴合，全部量化验证：
- k 空间：k²χ(k) 振幅/频率/相位贴合，k 3–9 corr≥0.99（9–11.5 段
  0.77 是数据 SNR 边界：该段 rms 仅 0.12，弱信号区）
- R 空间幅值：主峰高比 1.04，峰位差 0.04 Å（网格分辨率内）
- R 空间实部（最严格视图，此前从未检查）：corr 0.994
- 统计：三路径 Rf=0.0128 < 0.02，**优于报告的 0.013**

**参数层（方法边界，诚实记录）**——第一壳层 CN 与报告 4.3±0.5 的差距：
- 模型间漂移：单壳层 3.4–3.6 / 双壳层 2.4 / 三路径 2.74±0.6
- 原因不是 S0² 口径（约束 0.9 后不变），是**路径简并**：
  N@1.95 与 C@2.08 高度相关，振幅摊薄，单路径 CN 误差 ±0.29
- 方法边界：CN 是 EXAFS 最弱参数（S0²–CN 简并 + 路径简并），
  文献精度 ±15–25%；报告 4.3 的模型细节未知，无法完全对齐
- **交付纪律**：CN 报告必须带模型说明 + 误差棒；不同模型 CN 不同
  是常态不是错误，"模型族内一致"才可辩护

**σ² 审计否决三路径（08-18 终局）**：三路径 σ² 拟合到 2e-10±0.28
（过参数化红旗，物理失真）；σ² 固定 0.004/0.008 后混配 C 路径振幅
塌零（CN=0.00±0.08）——数据不支持第一壳层混配，三路径低 Rf 是
σ² 失真换的假象。**最终胜选 = 双壳层**（recipe 10：Pt-N 2.4@1.99 +
第二壳层 C 1.3@2.93，σ²=0.0027/0.0013 物理合理，Rf=0.0169，实部
corr 0.994）；opju/报告已切回。**教训：Rf 最优 ≠ 最好，σ² 跑到
0 是过参数化红旗，先审参数物理性再排名。**

**终版模型（作废）**：Pt-N 1.4@1.95 + Pt-C 1.4@2.08（= 报告"Pt-C/N"混配的
...
## 七点十二、全套第三层：RDF 序列 + EIS 真实对象（"自己努力"批次）

微信存储区三搜补齐（部分资产已抢入 data/raw/synchrotron_full/）：
- **1nm-RDF.zip**：gofr-0..400 时间序列（11 帧 × 101 点，r 0–10 Å，
  第 1/2/3 列 = r/g(r)/CN(r)）——同步辐射**全散射 RDF/PDF 线**（与 XAFS
  互补：PDF 给全部原子对）。recipe 14：切片+CN 叠图、r×t 热图、
  有序度演化（第一壳层峰 11.9→3.3、CN(2.5Å) 3.31→2.19，无序化）；
  帧参数物理含义（ps/温度/退火）提供方未注明，如实标注
- **拉曼结果.zip**（已抢存，未展开——谱图族 backlog）
- **商业情报**：扬州大学碳中和研究院测试合同（研趣代理，15300 元
  打包价）——同步辐射+表征外包的市场价格锚入库
- **EIS**：项目内真实 Origin 对象（obj3，slide5 panelC，61 点，列语义
  未随对象保存）；recipe 15 = 合成 Randles 闭环（手写复阻抗最小二乘，
  Rs/Rct/Q/n 四参数全恢复）+ Nyquist/Bode + 真实对象形状对照
- 三图 figcheck ALL PASS（figcheck 连抓两次自家字号违规后修复）
