...
- **RDF 时间序列线**（recipe 14，同步辐射第三技术块=全散射）：gofr
  11 帧全套分析（切片/热图/有序度+CN 演化），figcheck PASS
- **EIS 线**（recipe 15）：合成 Randles 四参数闭环恢复（10.0/500.4/
  2.02e-5/0.899 vs 真值全中）+ 真实 Origin 对象 Nyquist 对照（列语义
  TBC 诚实标注），figcheck PASS
- 商业价格锚：外包测试打包 15300 元（合同证据）入商业化文档素材
- figcheck 两次抓到自家 log 刻度上标 4.6pt——tick 7.5pt 后全过
  （工具连续自证价值）
